...
import { fetchDashboardInfo } from "../../../actions/api";
...
const DataStatsOne: React.FC<dataStats> = () => {
  ...
  const [gridConsumption, setGridConsumption] = useState<number>(0);
  const [solarEnergySold, setSolarEnergySold] = useState<number>(0);
  const [savings, setSavings] = useState<number>(0);
  const [solarConsumption, setSolarConsumption] = useState<number>(0);
    ...
    const fetchData = async () => {
      try {
        ...
        if (data && data.user) {
          ...
          const dashboardData = await fetchDashboardInfo(email, timestamp);
          // console.log("data info:", dashboardData);

          if (dashboardData) {
            setGridConsumption(dashboardData.grid_consumption);
            setSavings(dashboardData.total_savings);
            setSolarConsumption(dashboardData.solar_consumption);
            setSolarEnergySold(dashboardData.solar_energy_sold);
          }
        }
      } catch (error) {
  ...
  const updatedDataStatsList = [
    {
      ...dataStatsList[0],
      value: `${(gridConsumption/1000).toFixed(2)} kW`,
    },
    {
      ...dataStatsList[1],
    ...
    {
      ...dataStatsList[3],
      value: `${(solarConsumption/1000).toFixed(2)} kW`,
    },
  ];

  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-7.5">
      {updatedDataStatsList.map((item, index) => (
        <div
          key={index}
